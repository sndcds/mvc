export default class Router {
     ...
     */
    handlePopState() {
        this.loadRouteByName(event.state.slug, false)
    }

    /**
     * Get a route by its name.
     *
     * @param {string} slug - Name of route to find.
     * @return {object} The route or null.
     */
    getRouteByName(slug) {
        return this.routes.find((obj) => obj.slug === slug)
    }

    /**
     * Load a route by its name.
     *
     * @param {string} slug - Name of route to load.
     * @param {boolean} pushFlag - If true, new state will be pushed to window.history
     */
    loadRouteByName(slug, pushFlag = true) {
        this.loadRoute(this.getRouteByName(slug), pushFlag)
    }
    ...
    loadRoute(route, pushFlag = true) {
        if (route !== null) {
            const container = document.querySelector(route.injectionPoint) // Todo, helper nutzen und root von außen!
            ...
            if (typeof route.handler === 'function') {
                ...
                if (pushFlag === true) {
                    const state = {
                        ...
                        title: route.title,
                        path: route.path,
                        injectionPoint: route.injectionPoint
                    }
